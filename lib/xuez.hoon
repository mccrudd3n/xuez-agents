::  lib/xuez.hoon
::
/-  *xuez-wallet, json-rpc, xp=xuez-provider
/+  bip32, xz=xuez
=,  secp:crypto
=+  ecc=secp256k1
|%
::
::  Formerly lib/xuez-wallet.hoon
::
::
++  defaults
  |%
...
++  from-xpub
  |=  $:  =xpub:xz
      ...
      ==
  ^-  walt
  =/  [=bipt =network]  (xpub-type:xz xpub)
  :*  xpub
      network
      fprint
...
++  txb
  ...
  ++  vbytes
    ^-  vbytes:xz
    %+  add  overhead-weight:xz
    %+  add
      %+  roll
      (turn txis.t |=(t=txi (input-weight:xz bipt.hdkey.t)))
      add
    %+  roll
      (turn txos.t |=(t=txo (output-weight:xz (get-bipt:adr:xz address.t))))
    add
  ++  tx-data
    |^
    ^-  data:tx:xz
    ...
    ++  txo-data
      |=  =txo
      :-  (to-script-pubkey:adr:xz address.txo)
      value.txo
    --
  ::
  ++  get-txid
    ^-  txid
    (get-id:txu:xz tx-data)
  ::
  ++  get-rawtx
    (basic-encode:txu:xz tx-data)
  ...
  ++  to-psbt
    ...
    ?:  (lth (lent ins) (lent txis.t))
      ~
    =/  outs=(list out:psbt:xz)
      %+  turn  txos.t
      |=(=txo [address.txo hk.txo])
    `(encode:pbt:xz %.y get-rawtx get-txid ins outs)
  --
...
++  wad
  ...
  ++  mk-address
    |=  =idx:xz
    ^-  address:xz
    (from-pubkey:adr:xz bipt.w network.w (pubkey idx))
  ...
  ::
  ++  nixt-address
    ^-  (trel address:xz idx:xz walt)
    =/  addr  (mk-address nixt-idx)
    ~|  "lib/xuez-wallet-store: get-next-address: nixt shouldn't be blank"
  ...
  ++  bump-nixt
    |^  ^-  walt
    =/  new-idx=idx:xz  +(nixt-idx)
    |-  ?>  (lte new-idx max-index)
    =+  addr=(mk-address new-idx)
    =/  =addi
      %+  ~(gut by wach.w)  addr
      [%.n chyg new-idx *(set utxo:xz)]
    ?.  used.addi
      %=  w
...
|_  [w=walt eny=@uvJ last-block=@ud payee=(unit ship) =feyb txos=(list txo)]
  ...
  ++  dust-threshold
    |=  output-bipt=bipt:xz
    ^-  vbytes
    (mul dust-sats (input-weight:xz output-bipt))
  ::
  ++  target-value
    ^-  sats
  ...
  ++  base-weight
    ...
    %+  roll
      %+  turn  txos
      |=(=txo (output-weight:xz (get-bipt:adr:xz address.txo)))
    add
  ::
  ++  total-vbytes
    |=  selected=(list insel)
    ^-  vbytes
    %+  add  base-weight
    (mul (input-weight:xz bipt.w) (lent selected))
  ...
  ++  net-value
    |=  val=sats
    ^-  sats
    =/  cost  (mul (input-weight:xz bipt.w) feyb)
    ?:  (lte val cost)  0
    (sub val cost)
  ::
  ...
  ++  with-change
    ...
    =/  new-fee=sats                   ::  cost of this tx + one more output
      (mul feyb (add (output-weight:xz bipt.w) vbytes.u.tb))
    ?.  (gth excess new-fee)
      [tb ~]
  ...
  ++  select-utxos
    ...
    ?.  %+  levy  txos
        |=  =txo
        %+  gth  value.txo
        (dust-threshold (get-bipt:adr:xz address.txo))
    ...
    ++  to-insels
      ...
      |=(=utxo:xz [utxo chyg.addi idx.addi])
    --
  ::  single-random-draw
  ...
  ++  single-random-draw
    ...
    =|  [select=(list insel) total=sats:xz]
    |-
    ?:  =(~ is)  ~
...
::
++  gen-request
 |=  [=host-info:xp ract=action:rpc-types:xp]
  ^-  request:http
  %+  rpc-action-to-http
...
++  rpc
  ...
  ++  parse-result
    ...
    ++  address-info
      %-  ot
      :~  [%address (cu from-cord:adr:xz so)]
          [%utxos (as utxo)]
          [%used bo]
          [%block ni]
      ==
    ...
    %-  ot
      :~  ['tx_pos' ni]
          ['tx_hash' (cu from-cord:hxb:xz so)]
          [%height ni]
          [%value ni]
    ...
    ++  block-info
      ...
      :~  [%block ni]
          [%fee (mu ni)]
          [%blockhash (cu from-cord:hxb:xz so)]
          [%blockfilter (cu from-cord:hxb:xz so)]
      ==
    --
  --
::
++  rpc-action-to-http
  |=  [endpoint=@t ract=action:rpc-types:xp]
  |^  ^-  request:http
  ?-  -.ract
      %get-address-info
    %-  get-request
    %+  mk-url  '/addresses/info/'
    (to-cord:adr:xz address.ract)
    ::
      %get-tx-vals
    %-  get-request
    %+  mk-url  '/gettxvals/'
    (to-cord:hxb:xz txid.ract)
    ::
      %get-raw-tx
    %-  get-request
    %+  mk-url  '/getrawtx/'
