...
++  parse-filter
  |=  filter=hexb:xz
  ^-  [n=@ux gcs-set=bits:xz]
  =/  n  n:(de:csiz:xzu filter)
  =/  lead=@  ?:(=(1 wid.n) 1 +(wid.n))
  :-  dat.n
  [(mul 8 (sub wid.filter lead)) `@ub`dat:(drop:byt:xzu lead filter)]
...
++  to-key
  ...
  %+  take:byt:xzu  16
  %-  flip:byt:xzu
  (from-cord:hxb:xzu (crip blockhash))
...
::
++  match
  |=  [filter=hexb:xz k=byts targets=(list byts)]
  ^-  ?
  =/  [p=@ m=@]  [p:params m:params]
  =/  [n=@ux gcs-set=bits:xz]  (parse-filter filter)
...
++  all-match
  ...
  =/  [p=@ m=@]  [p:params m:params]
  =/  [n=@ux gcs-set=bits:xz]  (parse-filter filter)
  =/  target-map=(map @ hexb:xz)
    %-  ~(gas by *(map @ hexb:xz))
    %+  turn  targets
    |=(t=hexb:xz [(to-range:hsh t (mul n m) k) t])
