::  xuez-wallet
::
::  Note: Certain XUEZ Reference has not been chanced for "file naming" conversion 
::
::  Scrys
::  x/scanned: (list xpub) of all scanned wallets
::  x/balance/xpub: balance (in sats) of wallet
/-  *xuez-wallet, xp=xuez-provider
/+  dbug, default-agent, xl=xuez, xz=xuez, bip32
|%
++  defaults
  |%
  ++  params
...
+$  state-0
  $:  %0
      prov=(unit provider)
      walts=(map xpub:xz walt)
      =xuez-state
      =history
      curr-xpub=(unit xpub:xz)
      =scans
      =params
      feybs=(map ship sats)
      =piym
...
++  on-agent
  ...
  ?+  -.sign  (on-agent:def wire sign)
    ...
    =^  cards  state
      ?+  p.cage.sign  `state
          %xuez-provider-status
        (handle-provider-status:hc !<(status:xp q.cage.sign))
        ::
          %xuez-provider-update
        (handle-provider-update:hc !<(update:xp q.cage.sign))
      ==
  [cards this]
  ==
::
...
++  handle-command
  ...
  ?-  -.comm
    ...
    :*  %pass  /set-provider/[(scot %p provider.comm)]
          %agent  [provider.comm %xuez-provider]  %watch  /clients
      ==
    :_  state(prov [~ provider.comm %.n])
    ?~  prov  ~[sub-card]
    :~  [%pass /set-provider/[(scot %p host.u.prov)] %agent [host.u.prov %xuez-provider] %leave ~]
        sub-card
    ==
    ::
      %set-current-wallet
    ...
    =+  signed=(from-cord:hxb:xz txhex.comm)
    =/  tx-match=?
      ?~  poym  %.n
      =((get-id:txu:xz (decode:txu:xz signed)) ~(get-txid txb:xl u.poym))
    :-  ?.  tx-match
...
++  handle-action
  ...
  ?-  -.act
    ...
    ?~  curr-xpub  ~|("xuez-wallet-hook: no curr-xpub set" !!)
    =+  feyb=(~(gut by feybs) src.bowl ?~(fee.xuez-state fee:defaults u.fee.xuez-state))
    |^
    =^  tb=(unit txbu)  state
    ...
    ::
    ++  generate-txbu
      |=  [=xpub:xz payee=(unit ship) feyb=sats txos=(list txo)]
      ^-  [(unit txbu) _state]
      ...
      =/  [tb=(unit txbu) chng=(unit sats)]
        %~  with-change  sut:xl
        [u.uw eny.bowl block.xuez-state payee feyb txos]
      ...
      ?~  chng  [tb state]
      =/  [addr=address =idx w=walt]
        ~(nixt-address wad:xl u.uw %1)
      :-  `(~(add-output txb:xl u.tb) addr u.chng `(~(hdkey wad:xl w %1) idx))
...
++  handle-internal
  ...
  ?-    -.intr
    ...
    :_  state
    =+  pb=~(to-psbt txb:xl u.poym)
    ?~  pb  ~
    =+  vb=~(vbytes txb:xl u.poym)
    =+  fee=~(fee txb:xl u.poym)
    ...
    ++  poym-has-txid
      ...
      ?~  poym  %.n
      ?~  signed-tx.u.poym  %.n
      =(txid (get-id:txu:xz (decode:txu:xz u.signed-tx.u.poym)))
    ...
    ::
    ++  add-history-entry
      |=  [ti=info:tx =xpub:xz payer=ship payee=(unit ship) vout=@ud]
      ^-  ^history
      =/  =hest
        :*  xpub
            txid.ti
...
++  handle-provider-status
  ...
  ++  on-connected
    ...
    :_  %_  state
            prov  `p(connected %.y)
            xuez-state  [block fee now.bowl]
        ==
    ?:  ?|(?!(connected.p) (lth block.xuez-state block))
      ;:  weld
  ...
  ++  retry-poym
    ...
    %+  turn  txis.u.poym
    |=  =txi
    (poke-provider [%raw-tx ~(get-txid txb:xl u.poym)])
...
++  handle-provider-update
  ...
  ?-  -.p.upd
    ...
    ::
      %broadcast-tx
    ?~  poym  `state
    ?.  =(~(get-txid txb:xl u.poym) txid.p.upd)
      `state
    :_  state
...
++  handle-tx-info
  ...
  =/  =xpub
    xpub.w:(need (address-coords:xl (snag 0 ~(tap in our-addrs)) ~(val by walts)))
  ...
  ::
  ++  is-our-address
    |=(a=address ?=(^ (address-coords:xl a ~(val by walts))))
  --
++  set-curr-xpub
  |=  =xpub
  ^-  (quip card _state)
...
++  handle-address-info
  ...
  ^-  (quip card _state)
  =/  ac  (address-coords:xl address ~(val by walts))
  ?~  ac
    `state
  =/  [w=walt =chyg =idx]  u.ac
  =.  walts
    %+  ~(put by walts)  xpub.w
    %+  ~(update-address wad:xl w chyg)
      address
   [used chyg idx utxos]
...
::
++  req-scan
  |=  [b=batch =xpub:xz =chyg]
  ^-  (quip card _state)
  =/  w=walt  (~(got by walts) xpub)
  =/  as=(list [address [? ^chyg idx (set utxo)]])
    %+  turn  ~(tap in todo.b)
    |=(=idx [(~(mk-address wad:xl w chyg) idx) [%.n chyg idx *(set utxo)]])
  =.  w
    |-  ?~  as  w
    $(as t.as, w (~(update-address wad:xl w chyg) -.i.as +.i.as))
...
::
++  del-scanned
  |=  [b=batch =xpub:xz =chyg to-delete=idx]
  ^-  ^scans
...
++  poke-provider
  ...
  :*  %pass  /[(scot %da now.bowl)]
      %agent  [host.u.prov %xuez-provider]
      %poke   %xuez-provider-action  !>([act])
  ==
::
++  poke-peer
  ...
  :*  %pass  /[(scot %da now.bowl)]  %agent
      [target %xuez-wallet]  %poke
      %xuez-wallet-action  !>(act)
  ==
++  poke-internal
  |=  [intr=internal]
  ^-  card
  :*  %pass  /[(scot %da now.bowl)]  %agent
      [our.bowl %xuez-wallet]  %poke
      %xuez-wallet-internal  !>(intr)
  ==
::
++  is-broadcasting
  ^-  ?
