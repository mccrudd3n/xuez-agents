::  xuez-provider.hoon
::  Proxy that serves a XUEZ full node and ElectRS address indexer
::
::  Subscriptions: none
...
::
/-  *xuezcoin, json-rpc, *xuez-provider
/+  dbug, default-agent, xl=xuez, groupl=group, resource
|%
+$  versioned-state
    $%  state-0
    ==
...
++  on-init
  ^-  (quip card _this)
  ~&  >  '%xuez-provider initialized successfully'
  =|  wl=^whitelist
  :-  ~
  %_  this
      host-info
...
++  on-load
  |=  old-state=vase
  ^-  (quip card _this)
  ~&  >  '%xuez-provider recompiled successfully '
...
++  on-poke
  ...
  =^  cards  state
    ?+  mark  (on-poke:def mark vase)
        %xuez-provider-command
      ?>  (team:title our.bowl src.bowl)
      (handle-command:hc !<(command vase))
        %xuez-provider-action
...
++  on-watch
  ...
  ?.  (is-whitelisted:hc src.bowl)
    ~&  >>>  "xuez-provider: blocked client {<src.bowl>}"
    [~[[%give %kick ~ ~]] this]
  ~&  >  "xuez-provider: accepted client {<src.bowl>}"
  :-  do-ping:hc
...
++  send-status
  ...
  %-  ?:  ?=(%new-block -.status)
      ...
      same
  [%give %fact ~[/clients] %xuez-provider-status !>(status)]
::
++  send-update
  |=  =update
  ^-  card
  =+  c=[%give %fact ~[/clients] %xuez-provider-update !>(update)]
  ?:  ?=(%.y -.update)
...
++  do-ping
  ...
  :~  :*  %pass  /ping/[(scot %da now.bowl)]  %agent
          [our.bowl %xuez-provider]  %poke
          %xuez-provider-action  !>(act)
      ==
